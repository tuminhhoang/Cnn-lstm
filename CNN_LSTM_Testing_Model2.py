...
# Build a model 2 
model2 = Sequential()
model2.add(TimeDistributed(Conv2D(5, (5, 5), padding='same', input_shape=(ImageHeight, ImageWidth,ImageChannel), activation='relu'), input_shape=(num_timestep, ImageHeight, ImageWidth,ImageChannel)))

model2.add(TimeDistributed(MaxPooling2D(pool_size=(3, 3), strides = (3,3), padding='same')))
model2.add(TimeDistributed(Conv2D(5, (5, 5), padding='same', input_shape=(5, 30, 30), activation='relu')))

model2.add(TimeDistributed(MaxPooling2D(pool_size=(3, 3), strides = 3, padding='same')))
model2.add(TimeDistributed(Conv2D(5, (5, 5), padding='same', input_shape=(5, 10, 10), activation='relu')))

model2.add(TimeDistributed(MaxPooling2D(pool_size=(2, 2), strides = 2, padding='same')))
model2.add(TimeDistributed(Conv2D(10, (5, 5), padding='same', input_shape=(10, 5, 5), activation='relu')))

model2.add(TimeDistributed(keras.layers.BatchNormalization()))
model2.add(TimeDistributed(Flatten()))

# LSTM ------------------------------------------
model2.add(LSTM(hidden_layer1, input_shape=(num_timestep, 250), return_sequences=True))
...
# Build a model 2 
model3 = Sequential()
model3.add(TimeDistributed(Conv2D(5, (5, 5), padding='same', input_shape=(ImageHeight, ImageWidth,ImageChannel), activation='relu'), input_shape=(num_timestep, ImageHeight, ImageWidth,ImageChannel)))

model3.add(TimeDistributed(MaxPooling2D(pool_size=(3, 3), strides = (3,3), padding='same')))
model3.add(TimeDistributed(Conv2D(5, (5, 5), padding='same', input_shape=(5, 30, 30), activation='relu')))

model3.add(TimeDistributed(MaxPooling2D(pool_size=(3, 3), strides = 3, padding='same')))
model3.add(TimeDistributed(Conv2D(5, (5, 5), padding='same', input_shape=(5, 10, 10), activation='relu')))

model3.add(TimeDistributed(MaxPooling2D(pool_size=(2, 2), strides = 2, padding='same')))
model3.add(TimeDistributed(Conv2D(10, (5, 5), padding='same', input_shape=(10, 5, 5), activation='relu')))

model3.add(TimeDistributed(keras.layers.BatchNormalization()))
model3.add(TimeDistributed(Flatten()))

# LSTM ------------------------------------------
model3.add(LSTM(hidden_layer1, input_shape=(num_timestep, 250), return_sequences=True))
...
######################################################################
# Load Model
model1.load_weights("CNN_LSTM_Model1_D1_update.h5")
model2.load_weights("CNN_LSTM_Model1_D2_update.h5")
model3.load_weights("CNN_LSTM_Model1_D3_update.h5")
...
for CntTest in xrange(num_timestep,NumTestPoints):
    ...
    if Area_Choice[0] == 1: # Area 1
        PredictedArray1 = model1.predict(TestingInput)    # Prediction 
        print "Database 1 ... "
        print(PredictedArray1)
        
    if Area_Choice[1] == 1: # Area 2
        print "Database 2 ... "
        PredictedArray2 = model2.predict(TestingInput)    # Prediction  
        print(PredictedArray2)

    if Area_Choice[2] == 1: # Area 3
        print "Database 3 ... "
        PredictedArray3 = model3.predict(TestingInput)    # Prediction 
        print(PredictedArray3)
    
    # Check 3 Areas
    Sum_Area = Area_Choice[0] + Area_Choice[1] + Area_Choice[2]
    if Sum_Area == 1: # Only 1 area
        if Area_Choice[0] == 1: # Area 1
            Get_Locations(OutLoc, First_Loc, PredictedArray1, num_timestep, File1) 
        if Area_Choice[1] == 1: # Area 2
            Get_Locations(OutLoc, First_Loc, PredictedArray2, num_timestep, File1) 
        if Area_Choice[2] == 1: # Area 3
            Get_Locations(OutLoc, First_Loc, PredictedArray3, num_timestep, File1) 
    else: # Multiple areas
      #  print(Area_Choice)
        PredictedArray =  np.zeros((1,num_timestep,2))
        if Area_Choice[1] == 1: #1 and 2
            for kk in xrange(1, num_timestep):
                PredictedArray[0, kk, 0] = (PredictedArray1[0, kk, 0] + PredictedArray2[0, kk, 0])/2
                PredictedArray[0, kk, 1] = (PredictedArray1[0, kk, 1] + PredictedArray2[0, kk, 1])/2 
        if Area_Choice[2] == 1: #1 and 3
            for kk in xrange(1, num_timestep):
                PredictedArray[0, kk, 0] = (PredictedArray1[0, kk, 0] + PredictedArray3[0, kk, 0])/2
                PredictedArray[0, kk, 1] = (PredictedArray1[0, kk, 1] + PredictedArray3[0, kk, 1])/2   
